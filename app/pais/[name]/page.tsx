...
export default async function CountryPage({ params: { name } }: { params: {name: string }} ){
  ...
  <section className="flex flex-col container p-5 sm:p-0">

  <h1 className="text-5xl text-center font-bold text-gray-800 my-16">{country.translations.por.common}</h1>

    <Link className="flex items-center py-2 pl-2 gap-1 cursor-pointer" href="/">
      
      <Image src="/arrow-back.svg" alt="icone de voltar" width={15} height={15} />Voltar </Link>

    <article className="flex md:flex-row flex-col justify-between min-w-full p-5 sm:p-10 bg-white rounded-xl gap-1">
...

      {
        ...
        (
          ...
          <b>Capital:</b> {country.capital}</h2>
        )
      }
        ...
        <h2 className="text-xl text-gray-800 mt-3">
          <b>População:</b> {formatter.format(country.population)}</h2>
      ...
      <div className="relative h-48 my-2 md:h-auto sm:w-96 fill shadow-md md:order-last order-first">
    ...
    <section>
      <h3 className="mt-12 text-2xl font-semibold text-gray-800 pb-3">Países que fazem fronteira</h3>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 w-full container gap-2">
        ...
        <CountryCard key={"fronteira" + i} {... border}/> ) )}
      </div>
    </section>
    </section>
